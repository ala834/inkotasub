  ...
  try {
    ...
    if (isSubpadiConfigured()) {
      ...
      if (subpadiResult.success) {
        providerResult = { success: true, message: "Subscription successful", providerUsed: 'subpadi', fallbackAttempted: false, rawResponse: subpadiResult.rawResponse };
      } else {
        ...
        providerResult = {
          success: smeplugResult.success, message: smeplugResult.success ? "Subscription successful" : "Subscription failed",
      ...
      const smeplugResult = await withMetrics('smeplug', 'cable', () => smeplugPurchaseCable(providerCode, smartCardNumber, planId, costPrice), r => r.success);
      providerResult = { success: smeplugResult.success, message: smeplugResult.success ? "Subscription successful" : "Subscription failed", providerUsed: 'smeplug', fallbackAttempted: false, rawResponse: smeplugResult.rawResponse };
    }
